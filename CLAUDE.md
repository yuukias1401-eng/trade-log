...
### チャートタブ（拡大表示）
ウォッチリストカードに「チャート拡大」ボタンを追加し、押すと新しい「📊 チャート」タブに
ジャンプして同じ銘柄のフルスクリーン表示（MACD・RSI含む）を見れる機能。
- 新タブ追加: ウォッチリスト / 履歴 / 統計 / 📊チャート
- チャートタブ: 上部に銘柄セレクタ + 全幅大きめのローソク足
- インジケーター: MA25・MA75（まず実装済み）、RSI(14)サブチャート、MACD（将来）
- ウォッチリストカードの「チャート拡大」→ チャートタブに遷移して自動で当該銘柄を表示
